 ...
 * them.
 */
import { blobPath, box, polygon, taper } from "../shape.ts";
...
// Squish extensions: single continuous outlines keep every material and export
// on the same source geometry. Eyes use the existing expression model.
function localPath(b:Body, commands:(string|number)[]):string {
 let axis=0;return commands.map(v=>{if(typeof v==='string'){axis=0;return v}return String((axis++%2?b.cy+v*b.ry:b.cx+v*b.rx).toFixed(3))}).join(' ');
}
export const ghost:Shape={name:'ghost',core:.88,face:shrunk(.7),body:(_t,b)=>{b.n=2;b.rot=0},path:b=>localPath(b,[
 'M',-1,.72,'C',-.96,.2,-1,-.38,-.68,-.77,'C',-.36,-1.14,.36,-1.14,.68,-.77,'C',1,-.38,.96,.2,1,.72,
 'Q',.96,1.02,.71,.79,'Q',.47,.58,.25,.87,'Q',0,1.1,-.25,.87,'Q',-.47,.58,-.71,.79,'Q',-.96,1.02,-1,.72,'Z'])};
export const monster:Shape={name:'monster',core:.86,face:shrunk(.7),body:(_t,b)=>{b.n=2;b.rot=0},path:b=>{
 const p=[[-.96,-.3],[-.84,-1.28],[-.39,-.73],[0,-.86],[.39,-.73],[.84,-1.28],[.96,-.3],[1,.35],[.65,.88],[0,1],[-.65,.88],[-1,.35]];
 const mid=(a:number[],c:number[])=>[(a[0]+c[0])/2,(a[1]+c[1])/2];const out:(string|number)[]=['M',...mid(p.at(-1)!,p[0])];for(let i=0;i<p.length;i++)out.push('Q',...p[i],...mid(p[i],p[(i+1)%p.length]));out.push('Z');return localPath(b,out);
}};
...
export const codex:Shape={name:'codex',core:.82,body:(_t,b)=>{b.n=4;b.rot=0;b.ry=b.rx*.9;b.cy-=b.ry*.12},face:b=>({cx:b.cx,cy:b.cy-b.ry*.08,rx:b.rx*.58,ry:b.ry*.32}),path:b=>localPath(b,[
 'M',-.68,.43,
 'C',-.94,.40,-1.02,.19,-.94,.02,
 'C',-1.13,-.19,-1,-.49,-.79,-.55,
 'C',-.8,-.78,-.61,-.94,-.39,-.90,
 'C',-.30,-1.17,-.04,-1.19,.10,-1.04,
 'C',.31,-1.18,.53,-1.02,.55,-.84,
 'C',.81,-.85,.92,-.64,.88,-.48,
 'C',1.08,-.37,1.09,-.12,.96,.03,
 'C',1.02,.28,.88,.44,.66,.46,
 'Q',.53,.48,.45,.49,
 'C',.51,.53,.65,.65,.67,.83,
 'Q',.70,1.01,.58,1.02,'Q',.47,1.03,.44,.83,
 'Q',.42,1.00,.31,1.05,
 'L',.31,1.22,'Q',.30,1.34,.18,1.34,
 'Q',.06,1.34,.06,1.22,'L',.06,1.10,
 'L',-.07,1.10,'L',-.07,1.22,'Q',-.08,1.34,-.20,1.34,
 'Q',-.32,1.34,-.32,1.22,'L',-.32,1.05,
 'Q',-.43,1.00,-.44,.83,'Q',-.48,1.03,-.59,1.02,
 'Q',-.70,1.01,-.67,.83,'C',-.65,.66,-.54,.54,-.46,.49,
 'Q',-.58,.47,-.68,.43,'Z'])};
